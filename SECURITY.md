...
Releases from `cachekit-core-v0.4.0` onward carry a CycloneDX 1.6 SBOM as a
release asset named `cachekit-core-sbom.json`, generated by `cargo-sbom` from
the released tag. Earlier releases predate this and have no SBOM asset.

```bash
gh release download cachekit-core-v0.4.0 --repo cachekit-io/cachekit-core \
  --pattern cachekit-core-sbom.json
```

Separately, the publish workflow attests an SBOM of the packaged crate via
[`actions/attest-sbom`](https://github.com/actions/attest-sbom). That attestation
is the cryptographically verifiable artifact — the release asset above is a
convenience copy, generated by the same tool in the same format but not itself
signed:
