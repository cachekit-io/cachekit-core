...
mod security_tests {
    ...
    fn test_constant_time_operations_baseline() {
        ...
        // specialized tools like dudect). This test establishes baseline timing
        // to detect obvious timing leaks.
        ...
        for _ in 0..100 {
            ...
            let _ = encryptor
                .encrypt_aes_gcm(&plaintext_zeros, &key, aad)
                .expect("Encryption should succeed");
            timings_zeros.push(start.elapsed().as_nanos());
        }
        ...
        for _ in 0..100 {
            let start = Instant::now();
            let _ = encryptor
                .encrypt_aes_gcm(&plaintext_ones, &key, aad)
                .expect("Encryption should succeed");
            timings_ones.push(start.elapsed().as_nanos());
        }

        // Calculate median timings (more robust than mean for small samples)
        timings_zeros.sort_unstable();
        timings_ones.sort_unstable();

        let median_zeros = timings_zeros[timings_zeros.len() / 2];
        let median_ones = timings_ones[timings_ones.len() / 2];

        // Calculate relative difference
        let diff = if median_zeros > median_ones {
            (median_zeros - median_ones) as f64 / median_zeros as f64
        } else {
            (median_ones - median_zeros) as f64 / median_ones as f64
        };

        // Timings should be similar (within 20% - allows for system variance)
        // A significant difference would indicate potential timing side-channel
        assert!(
            diff < 0.20,
            "Timing difference too large: {:.1}% (zeros: {}ns, ones: {}ns) - possible timing leak",
            diff * 100.0,
            median_zeros,
            median_ones
        );
        ...
        println!(
            "✓ Timing baseline: zeros={}ns, ones={}ns, diff={:.1}%",
            median_zeros,
            median_ones,
            diff * 100.0
        );
